...
void model::eval_chpi_h_ring(aliphatic_carbon_attribute& c, ring_attribute& r, pr& dH_minusTdS, bool fast, bool is_non_cache){
	...
	VINA_FOR(i, c.num_h_neighbors){
		...
		interacting_h_i.push_back(i);
	}

	sz num_interacting_h = interacting_h_i.size();	
	bool proper_interaction = false;

	VINA_FOR(i, num_interacting_h){
		sz h_i = interacting_h_i[i];
		sz h_index = h_neighbor_indices[h_i];
		//if (ligand_aliphatic && has(this->adjusted_atom_indices, h_index)) continue;

		//vec* hv = (ligand_aliphatic) ? this->adjusted_coord_ptrs[h_index] : h_coords[h_i];
		vec* hv = h_coords[h_i];
		vec ch_bond = (*hv) - (*c_coord);
		//std::cout << "In non cache CH bond length: " << magnitude(ch_bond) << std::endl;

                fl chndp = ch_bond * normal; //CH-Normal dot product
		fl chn_cosine_abs = std::abs(chndp / magnitude(ch_bond));
		...
		//VINA_FOR(j, r.num_ring_atoms)
			...
			fl sum_vdw_radii = xs_vdw_radii[all_atom_ptrs[j]->xs] + 1.1; //1.1 A is the VDW radii of hydrogen we use. 
			//Comment out below for no H repulsion
			if (r < sum_vdw_radii){
				pr repulsion_e_dor = (fast) ? this->eval_repulsion_fast(r, sum_vdw_radii) : this->eval_repulsion_deriv(r, sum_vdw_radii);
				fl repulsion_e = repulsion_e_dor.first; fl repulsion_dor = repulsion_e_dor.second;
				//dH += repulsion_e;
				//this_pair_e_dor.first += repulsion_e; this_pair_e_dor.second += repulsion_dor;

				dH -= repulsion_e;
				this_pair_e_dor.first -= repulsion_e; this_pair_e_dor.second -= repulsion_dor;
			}

			//pr e_dor = (fast) ? this->eval_chpi_enthalpy_h_fast(r2, chn_cosine_abs) : this->eval_chpi_enthalpy_h_deriv(r2, chn_cosine_abs);
			pr e_dor = (fast) ? this->eval_chpi_enthalpy_h_fast(r, chn_cosine_abs, num_interacting_h) : this->eval_chpi_enthalpy_h_deriv(r, chn_cosine_abs, num_interacting_h);
			fl& h_ar_dH = e_dor.first; fl& h_ar_dor = e_dor.second;
...
pr model::eval_chpi_enthalpy_h_deriv(fl r, fl chn_cos_abs, sz num_interacting_h){
...
pr model::eval_chpi_enthalpy_h_fast(fl r, fl chn_cosine_abs, sz num_interacting_h){
	...
	return pr(this->chpi_h_fast[i], 0);
}
