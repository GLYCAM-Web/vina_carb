...
//std::cout << "Prec n value: " << n << std::endl;
	...
	fl eval_fast(sz type_pair_index, fl r2) const {
		...
		//return data(type_pair_index).eval_fast_new(r2);
	}
	pr eval_deriv(sz type_pair_index, fl r2) const {
		assert(r2 <= m_cutoff_sqr);
		return data(type_pair_index).eval_deriv(r2);
		//return data(type_pair_index).eval_deriv_new(r2);
	}
	...
	//Make calculate rs public. Yao 20230826
...
public:
	fl factor; //Made public for model to access
private:
	fl m_cutoff_sqr;
	sz n;
