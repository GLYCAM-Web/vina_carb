...
struct rigid_body : public atom_frame {
	...
	//std::cout << "Rigid body set conf called" << std::endl;
		origin = c.position;
...
struct segment : public axis_frame {
	...
	void set_conf(const frame& parent, const atomv& atoms, vecv& coords, flv::const_iterator& c) {
		const fl torsion = *c;
	//std::cout << "Segment set conf ligand flv iterator value is " << torsion << std::endl;
		++c;
...
void branches_set_conf(std::vector<T>& b, const frame& parent, const atomv& atoms, vecv& coords, flv::const_iterator& c) {
	VINA_FOR_IN(i, b){
		//std::cout << "Branch set conf b = " << i << std::endl;
...
struct heterotree {
	...
	//std::cout << "Heterotree set conf called" << std::endl;
