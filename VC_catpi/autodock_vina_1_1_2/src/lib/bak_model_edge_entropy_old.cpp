...
void model::DetectAromaticCycles(resv& residues, ring_info& ring_attributes, bool is_ligand){
	...
	VINA_FOR_IN(i, residues){
		...
		VINA_FOR_IN(j, ar_cycles_this_residue){
			...
			ring_attributes.push_back(ring_attribute(this, this_cycle, atom_indices, temp_coord_ptrs, is_ligand, ring_hydrogens, ring_h_indices, ring_h_coords, ring_h_heavy_neighbor_indices, ring_h_heavy_neighbor_coords, &(adjusted_coord_ptrs), false, temp_atom_ptrs));
		}
	}
	return;
}
...
void model::eval_chpi_h_ring(aliphatic_carbon_attribute& c, ring_attribute& r, pr& dH_minusTdS, bool fast, bool is_non_cache){
	...
	flv h_centroid_dist(c.num_h_neighbors, 0);
	std::vector<flv> h_ring_dists(c.num_h_neighbors, flv(r.num_ring_atoms, 0));
	std::vector<bool> h_interacting(c.num_h_neighbors, false);
	...
	bool this_c_interacting = false;
	VINA_FOR_IN(i, h_interacting){
		if (h_interacting[i]){
			this_c_interacting = true;
			break;
		}
	}
	if (!this_c_interacting) return;

	sz closest_h_i = 0; fl min_hc_dist = max_fl;
	VINA_FOR_IN(i, h_centroid_dist){
		fl& hc_dist = h_centroid_dist[i];
